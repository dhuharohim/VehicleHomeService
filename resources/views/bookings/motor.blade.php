...
@section('section')
                ...
                <div class="col-12">
                            <div class="card">
                                <div class="card-body shadow-lg">
                                    <center>
                                        <form action="{{ route('booking.store') }}" class="" method="POST">
                                            @csrf
                                            <br>
                                            <div class="form-group">
                                                ...
                                                <select name="merk"
                                                    ...
                                                    id="parent" required>
                                                    <option value="" class="text-center">--Pilih Merk Motor--</option>
                                                    @foreach ($merk as $mer)
                                                        <option value="{{ $mer->id }}">{{ $mer->merk_motor }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <br>
    ...
    <script>
        $("#parent").change(function() {
            ...
            $("#child1").html('')
            var url = '/bookings/tipe/' + $(this).val();
            $.ajax({
                type: "get",
                url: url,
                success: function(response) {
                    $("#child1").append(`<option value="" class="text-center">--Pilih Model Motor--</option>`)
                    response.forEach(function(params) {
                        $("#child1").append(`<option value="${params.id}">${params.tipe_motor}</option>`)
        ...
        $("#child1").change(function() {
            console.log($(this).val());
            $("#child2").html('')
            var url = '/bookings/varian/' + $(this).val();
            $.ajax({
                type: "get",
                url: url,
                success: function(response) {
                    $("#child2").append(`<option value="" class="text-center">--Pilih Varian Motor--</option>`)
                    response.forEach(function(params) {
                        $("#child2").append(`<option value="${params.id}">${params.varian_motor}</option>`)
                        // console.log(params.tipe_motor);
                    });
                }
            });
